...
public enum HttpMessage {
  ...
  TOO_MANY_REQUEST(
      10, "too Many Request: try again after a few minutes.", HttpStatus.TOO_MANY_REQUESTS),
  BAD_SIGNATURE(
      11,
      "signature does not match, your payload may be edited or damaged.",
  ...
  ALREADY_EXISTED(
      200, "{resource} already existed, {identity} must be unique.", HttpStatus.BAD_REQUEST),
  ...
  ILLEGAL_RESOURCE_ID(203, "{resource} id must be in valid string format.", HttpStatus.BAD_REQUEST),
  ...
  FIND_ALL_BY_NO_CONTENT(
      207, "retrieve {resource} by {criteria} return no content.", HttpStatus.NO_CONTENT),
  FIND_ALL_ARCHIVED_NO_CONTENT(
      208, "retrieve archived {resource} return no content.", HttpStatus.NO_CONTENT),
  ...
  SIGN_TOKEN(
      211,
      "can not sign token: illegal claims or encrypt algorithm.",
      HttpStatus.INTERNAL_SERVER_ERROR),
  // @formatter:off
  PARSE_TOKEN(
      212,
      "can not parse token: token has been edited, expired or not published by us.",
      HttpStatus.INTERNAL_SERVER_ERROR), // @formatter:on
  // @formatter:off
  ILLEGAL_TOKEN(
      213,
      "illegal token: token has been edited, expired or not published by us.",
      HttpStatus.UNAUTHORIZED), // @formatter:on
  TOKEN_EXPIRED(214, "illegal token: token has been expired.", HttpStatus.UNAUTHORIZED),
  BAD_CREDENTIAL(
      215, "bad credentials: username or password does not match.", HttpStatus.UNAUTHORIZED),
  // @formatter:off
  MISSING_AUTHORIZATION(
      216,
      "unauthorized: missing authorization or x-refresh-token.",
      HttpStatus.UNAUTHORIZED), // @formatter:on
  TOKEN_BLOCKED(217, "token has been recalled: can not use this anymore.", HttpStatus.UNAUTHORIZED),
  DECODE_RESOURCE_ID(
      218,
      "can not decode resource id: make sure you provide a valid one.",
      HttpStatus.BAD_REQUEST),
  ENSURE_NOT_BAD_CREDENTIAL(219, "can not ensure token is not recalled.", HttpStatus.UNAUTHORIZED),
  TOKEN_NOT_SUITABLE(
      220, "access token and refresh token are not suitable.", HttpStatus.UNAUTHORIZED),
  RECALL_TOKEN(221, "refresh token not success: token not be recalled.", HttpStatus.UNAUTHORIZED),
  MISSING_HMAC_SIGNATURE(
      222, "hmac signature is missing, can not verify payload.", HttpStatus.BAD_REQUEST),
  ...
  NONCE_BLOCKED(
      225,
      "nonce has been used: your process can not start with this nonce.",
      HttpStatus.BAD_REQUEST),
  ENSURE_NOT_BAD_NONCE(
      226, "can not ensure nonce is not blocked.", HttpStatus.INTERNAL_SERVER_ERROR);
  ;
