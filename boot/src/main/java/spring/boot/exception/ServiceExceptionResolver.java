...
public class ServiceExceptionResolver {
  ...
  public ResponseEntity<Response<Object>> notFound(NoResourceFoundException exception) {
    ...
    var response =
        Response.builder().code(notFound.getCode()).message(notFound.getMessage()).build();
    return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
  }

  @ExceptionHandler(value = MethodArgumentNotValidException.class)
  public ResponseEntity<Response<Object>> illLegalPayload(
  ...
  public ResponseEntity<Response<Object>> paramNotReadable(
      MissingServletRequestParameterException exception) {
    HttpMessage badRequest = HttpMessage.ILL_LEGAL_PARAMETER;
    String message =
        MessageFormat.format("{0} is missing or not readable.", exception.getParameterName());
    var response = Response.builder().code(badRequest.getCode()).message(message).build();
    return ResponseEntity.badRequest().body(response);
  }
  ...
  public ResponseEntity<Response<Object>> paramNotReadable(
    ...
    HttpMessage badRequest = HttpMessage.ILL_LEGAL_PARAMETER;
    String message =
        MessageFormat.format("{0} must be in legal format.", exception.getPropertyName());
    var response = Response.builder().code(badRequest.getCode()).message(message).build();
    return ResponseEntity.badRequest().body(response);
  }

  @ExceptionHandler(value = HttpMessageNotReadableException.class)
  public ResponseEntity<Response<Object>> payloadNotReadable(
      HttpMessageNotReadableException exception) {
    HttpMessage badRequest = HttpMessage.ILL_LEGAL_PAYLOAD;
    var response =
        Response.builder().code(badRequest.getCode()).message(badRequest.getMessage()).build();
    return ResponseEntity.badRequest().body(response);
  }

  @ExceptionHandler(value = MissingRequestHeaderException.class)
  public ResponseEntity<Response<Object>> headerNotReadable(
      MissingRequestHeaderException exception) {
    HttpMessage badRequest = HttpMessage.ILL_LEGAL_HEADER;
    String message =
        MessageFormat.format("{0} is missing or not readable.", exception.getHeaderName());
    var response = Response.builder().code(badRequest.getCode()).message(message).build();
    return ResponseEntity.badRequest().body(response);
  }
  ...
  public ResponseEntity<Response<Object>> forbidden(AuthorizationDeniedException exception) {
    ...
    var response =
        Response.builder().code(forbidden.getCode()).message(forbidden.getMessage()).build();
  ...
  public ResponseEntity<Response<Object>> service(ServiceException exception) {
    HttpMessage causeBy = exception.getCausBy();
    Response<Object> response =
        Response.builder().code(causeBy.getCode()).message(causeBy.getMessage()).build();
